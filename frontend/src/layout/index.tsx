	...
	return (
		<aside className="h-full w-[248px] shrink-0 bg-white border-r border-line flex flex-col">
			...
			<div className="flex-1 overflow-y-auto py-3 px-3">
				{NAV_GROUPS.filter((g) => !g.adminOnly || TEACHER.isAdmin).map(
					(group) => (
						...
						<div className="absolute right-0 top-full mt-2 w-[320px] bg-white border border-line rounded-lg shadow-lg modal-anim z-40">
							...
							<div className="max-h-[300px] overflow-y-auto">
								{NOTIFICATIONS.map((n) => (
									<div
										key={n.id}
										className={`px-4 py-3 border-b border-line/60 ${n.unread ? "bg-primary-light/20" : ""}`}
									>
					...
					items={[
						{
							label: TEACHER.name,
							icon: "user",
							right: TEACHER.isAdmin ? "Admin" : "Teacher III",
						},
						{ separator: true },
						...(TEACHER.isAdmin
							? [
