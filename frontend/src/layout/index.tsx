...
export const NAV_GROUPS = [
  ...
  { label:'Admin', adminOnly:true, items: [
    { id:'admin',      label:'Admin Console', icon:'shield-check', badge:'PRO' },
  ]},
  { label:'System', items: [
  ...
  return (
    <aside className="h-full w-[248px] shrink-0 bg-white border-r border-line flex flex-col">
      <div className="px-4 py-4 border-b border-line flex items-center justify-between">
        <Logo size={16} />
        {onClose && (
          <button className="lg:hidden text-muted p-1" onClick={onClose} aria-label="Close menu">
            <Icon name="x" size={18}/>
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-3 px-3">
        {NAV_GROUPS.filter(g => !g.adminOnly || TEACHER.isAdmin).map(group => (
          <div key={group.label} className="mb-4">
            <div className="px-2 mb-1.5 text-[10px] font-semibold tracking-[0.12em] uppercase text-muted-light">{group.label}</div>
      ...
      <div className="px-3 pb-3 border-t border-line pt-3 flex flex-col gap-3">
        ...
        <div className="rounded-md border border-line bg-white p-2.5 flex items-center gap-2.5">
          <div className="relative">
            ...
            {TEACHER.isAdmin && (
              <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-gradient-to-br from-primary to-emerald-500 inline-flex items-center justify-center border-2 border-white" title="Administrator">
                <Icon name="shield" size={8} className="text-white"/>
              </span>
            )}
          </div>
          <div className="min-w-0 flex-1">
            <div className="text-[12.5px] font-semibold text-navy truncate flex items-center gap-1.5">{TEACHER.name}</div>
            <div className="text-[11px] text-muted truncate">{TEACHER.isAdmin ? TEACHER.roleLabel : TEACHER.school}</div>
          </div>
          <button className="text-muted hover:text-navy" aria-label="Account"><Icon name="chevron-up" size={14}/></button>
        </div>
      </div>
    </aside>
  );
...
export function TopBar({ route, setRoute, online, setOnline, onMenu, breadcrumbExtra = undefined }) {
  const titles = {
    ...
    sync:      { title:'Sync Center', crumb:['Home','System','Sync Center'] },
    settings:  { title:'Settings', crumb:['Home','System','Settings'] },
    'class-detail': { title:'Class Details', crumb:['Home','My Classes','Class Details'] },
    'student-profile': { title:'Student Profile', crumb:['Home','Students','Student Profile'] },
  ...
  return (
    <header className="h-16 bg-white border-b border-line px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30">
      <div className="flex items-center gap-3 min-w-0">
        <button className="lg:hidden p-1.5 rounded-md hover:bg-slate-100" onClick={onMenu} aria-label="Menu">
          <Icon name="menu" size={20}/>
        </button>
        <div className="min-w-0">
          <h1 className="text-[18px] font-semibold tracking-tight text-navy truncate">{breadcrumbExtra?.title || t.title}</h1>
          <div className="hidden sm:flex items-center gap-1.5 text-[11.5px] text-muted">
            {(breadcrumbExtra?.crumb || t.crumb).map((c,i,arr) => (
              <React.Fragment key={i}>
                <span className={i===arr.length-1?'text-navy/70 font-medium':''}>{c}</span>
                {i < arr.length-1 && <Icon name="chevron-right" size={11} className="text-muted-light"/>}
              </React.Fragment>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative">
          <button className="relative w-9 h-9 rounded-md hover:bg-slate-100 flex items-center justify-center" onClick={() => setNotifsOpen(o => !o)} aria-label="Notifications">
            ...
            {unread > 0 && <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 rounded-full bg-rose-500 text-white text-[10px] font-bold flex items-center justify-center">{unread}</span>}
          </button>
          {notifsOpen && (
            <div className="absolute right-0 top-full mt-2 w-[320px] bg-white border border-line rounded-lg shadow-lg modal-anim z-40">
              <div className="px-4 py-2.5 border-b border-line flex items-center justify-between">
                ...
                <button className="text-[11px] text-primary font-semibold">Mark all read</button>
              </div>
              <div className="max-h-[300px] overflow-y-auto">
                {NOTIFICATIONS.map(n => (
                  <div key={n.id} className={`px-4 py-3 border-b border-line/60 ${n.unread?'bg-primary-light/20':''}`}>
                    <div className="flex items-start gap-2">
                      <Icon name="bell" size={14} className="text-primary mt-0.5"/>
                      <div className="flex-1 min-w-0">
                        <div className="text-[12.5px] text-navy">{n.text}</div>
                        <div className="text-[11px] text-muted mt-0.5">{n.when}</div>
                      </div>
              ...
              <div className="px-4 py-2 text-center">
                <button className="text-[12px] text-muted hover:text-navy">View all activity</button>
              </div>
            </div>
          )}
        </div>
        ...
        <Dropdown
          align="right"
          trigger={
            <button className="flex items-center gap-2 hover:bg-slate-100 rounded-md px-1 py-1 tx" aria-label="Account">
              <div className="relative">
                <Avatar name={TEACHER.name} size="sm" />
                {TEACHER.isAdmin && (
                  <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-gradient-to-br from-primary to-emerald-500 inline-flex items-center justify-center border-2 border-white">
                    <Icon name="shield" size={6} className="text-white"/>
                  </span>
                )}
              </div>
              <Icon name="chevron-down" size={14} className="text-muted hidden sm:inline"/>
            </button>
          }
          items={[
            { label: TEACHER.name, icon:'user', right: TEACHER.isAdmin? 'Admin' : 'Teacher III' },
            { separator: true },
            ...(TEACHER.isAdmin ? [{ label: 'Admin Console', icon:'shield-check', onClick: () => setRoute('admin') }] : []),
            { label: 'Schedules',       icon:'calendar-days', onClick: () => setRoute('schedules') },
            { label: 'Profile & School', icon:'id-card', onClick: () => setRoute('settings') },
            { label: 'Sync Center',     icon:'refresh-cw', onClick: () => setRoute('sync') },
            { label: 'Settings',        icon:'settings', onClick: () => setRoute('settings') },
            { separator: true },
            { label: 'Sign out',        icon:'log-out', onClick: () => { window.location.href = '/signin'; } },
          ]}
        />
      </div>
    </header>
      ...
      {MOBILE_TABS.map(t => {
        ...
        return (
          <button
            ...
            onClick={() => t.id === 'more' ? openMore() : setRoute(t.id)}
          >
...
export function MoreSheet({ open, onClose, setRoute }) {
  ...
  const items = [
    ...
    { id:'schedules',label:'Schedules', icon:'calendar-days' },
    { id:'reports',  label:'Reports',   icon:'file-text' },
    ...(TEACHER.isAdmin ? [{ id:'admin', label:'Admin Console', icon:'shield-check' }] : []),
    { id:'sync',     label:'Sync Center', icon:'refresh-cw' },
    { id:'settings', label:'Settings',  icon:'settings' },
  ];
  return (
    <div className="lg:hidden fixed inset-0 z-[60]">
      ...
      <div className="absolute bottom-0 inset-x-0 bg-white rounded-t-xl p-4 modal-anim border-t border-line pb-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          {items.map(it => (
            <button key={it.id} onClick={() => { onClose(); setRoute(it.id); }} className="flex items-center gap-3 p-3 rounded-md border border-line bg-surface hover:bg-white press tx">
              <span className="w-8 h-8 rounded-md bg-primary-light text-primary inline-flex items-center justify-center"><Icon name={it.icon} size={16}/></span>
