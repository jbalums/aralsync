...
import {
  ...
  SubjectChip, gradeColor, SectionHeader, Btn,
  Tabs, Field, TextInput, Select,
  BADGE_STYLES,
} from '../components';
import {
  useClassLoads,
...
export function PageClasses() {
  ...
  const {
    ...
    defaultValues: { wwPct: 20, ptPct: 60, qaPct: 20, quarter: 'Q1', gradeLevel: 7 },
  });
  ...
  const onCreateSubmit = async (values: CreateFormValues) => {
    try {
      ...
      toast.push({ type: 'success', title: 'Class created', message: 'You can now take attendance and enter grades.' });
      setNewOpen(false);
      reset();
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
      toast.push({ type: 'error', title: 'Failed to create class', message: msg ?? 'Please try again.' });
    }
  };

  const filtered = classLoads.filter((c) => {
    if (gradeFilter === 'all') return true;
    return c.section.gradeLevel.includes(gradeFilter.replace('g', 'Grade '));
  });
  ...
  return (
    <div className="page-anim space-y-5">
          ...
          <div>
            ...
            {isLoading ? (
              ...
              <p className="text-[13px] text-muted mt-1">
                {classLoads.length} active {classLoads.length === 1 ? 'class' : 'classes'} · {classLoads.reduce((a, c) => a + c.studentCount, 0)} total students
              </p>
            )}
          </div>
          ...
          {['all', 'g7', 'g8', 'g9', 'g10'].map((f) => (
            <button
              ...
              className={`px-3 h-8 rounded-md text-[12.5px] font-semibold transition-colors ${
                gradeFilter === f ? 'bg-navy text-white' : 'bg-white text-navy border border-line hover:bg-surface'
              }`}
            >
              {f === 'all' ? 'All' : `Grade ${f.slice(1)}`}
            </button>
          ))}
          <span className="ml-auto text-[12px] text-muted">{filtered.length} shown</span>
        </div>
      </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filtered.map((c) => (
            <ClassLoadCard
              key={c.id}
              load={c}
              onView={() => void navigate({ to: '/app/classes/$classId', params: { classId: c.id } })}
              onAttendance={() => void navigate({ to: '/app/attendance' })}
            />
          ))}
        </div>
      )}
      ...
      <Modal
        open={newOpen}
        onClose={() => { setNewOpen(false); reset(); }}
        title="Create a new class load"
        subtitle="Fill in to add a new section to your assignments"
        width="max-w-2xl"
        footer={
          <>
            <Btn variant="ghost" onClick={() => { setNewOpen(false); reset(); }}>Cancel</Btn>
            <Btn
              variant="primary"
              icon="plus"
            ...
            >
              {isSubmitting || createMutation.isPending ? 'Creating…' : 'Create class'}
            </Btn>
          </>
        }
      >
        <form className="grid grid-cols-2 gap-3" noValidate>
          <Field label="Subject name" required error={errors.subjectName?.message}>
            <TextInput placeholder="e.g. Science" {...register('subjectName')}/>
          </Field>
          ...
          <Field label="Section name" required error={errors.sectionName?.message}>
            <TextInput placeholder="e.g. Mabini" {...register('sectionName')}/>
          </Field>
          ...
          <Field label="Quarter" required error={errors.quarter?.message}>
            <Select {...register('quarter')}>
              {(['Q1', 'Q2', 'Q3', 'Q4'] as const).map((q) => <option key={q}>{q}</option>)}
            </Select>
          </Field>

          <div className="col-span-2 mt-2">
            <div className="text-[12px] font-semibold text-navy mb-2">
              Component weights <span className="text-muted font-normal">(must total 100%)</span>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <Field label="Written Works %" required error={errors.wwPct?.message}>
                <TextInput type="number" min={1} max={98} {...register('wwPct')}/>
              </Field>
              <Field label="Performance Tasks %" required error={errors.ptPct?.message}>
                <TextInput type="number" min={1} max={98} {...register('ptPct')}/>
              </Field>
              <Field label="Quarterly Assessment %" required error={errors.qaPct?.message}>
                <TextInput type="number" min={1} max={98} {...register('qaPct')}/>
              </Field>
            </div>
            <ComponentWeightBar ww={ww ?? 0} pt={pt ?? 0} qa={qa ?? 0} height={10} className="mt-3"/>
            <div className={`text-[11px] mt-1 font-mono ${weightSum === 100 ? 'text-emerald-600' : 'text-red-500'}`}>
              Sum: {weightSum}% {weightSum === 100 ? '✓' : '(must be 100%)'}
            </div>
            {errors.qaPct?.message && (
              <p className="text-[12px] text-red-500 mt-1">{errors.qaPct.message}</p>
            )}
          </div>
        </form>
      </Modal>
...
function ClassLoadCard({
  ...
  return (
    <Card variant="interactive" className="overflow-hidden" onClick={onView}>
      ...
      <div className="p-4 sm:p-5">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[12px] text-muted mt-0.5 flex items-center gap-2 flex-wrap">
              <span className="inline-flex items-center gap-1">
                <Icon name="users" size={12}/>{load.studentCount} students
              </span>
              {load.roomNumber && (
                <>
                  <span>·</span>
                  <span className="inline-flex items-center gap-1">
                    <Icon name="map-pin" size={12}/>{load.roomNumber}
                  </span>
                </>
              )}
              {load.scheduleTime && (
                <>
                  <span>·</span>
                  <span className="inline-flex items-center gap-1">
                    <Icon name="clock" size={12}/>{load.scheduleTime}
                  </span>
                </>
              )}
            </div>
          </div>
          <Avatar name={`${load.section.name} ${load.subject.name}`} size="md" square/>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="text-[10.5px] text-muted font-mono mt-1">
            WW {Math.round(load.wwWeight * 100)}% / PT {Math.round(load.ptWeight * 100)}% / QA {Math.round(load.qaWeight * 100)}%
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <Btn
            ...
            onClick={(e: React.MouseEvent) => { e.stopPropagation(); onAttendance(); }}
          >
            Take attendance
          </Btn>
        </div>
...
export function PageClassDetail() {
  ...
  if (!cls) {
    return (
      <EmptyState icon="alert-circle" title="Class not found" message="This class load does not exist or you don't have access."/>
    );
  }
  ...
  return (
    <div className="page-anim space-y-5">
      <Card className="p-4 sm:p-5 overflow-hidden relative">
        <div className="absolute top-0 left-0 w-2 h-full" style={{ background: hue }}/>
        <div className="pl-3 flex items-start justify-between gap-3 flex-wrap">
          <div>
            <button
              onClick={() => void navigate({ to: '/app/classes' })}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {cls.roomNumber && (
                <span className="pill" style={{ background: '#F1F5F9', color: '#334155' }}>
                  <Icon name="map-pin" size={11}/>{cls.roomNumber}
                </span>
              )}
              {cls.scheduleTime && (
                <span className="pill" style={{ background: '#F1F5F9', color: '#334155' }}>
                  <Icon name="clock" size={11}/>{cls.scheduleTime}
                </span>
              )}
            </div>
      ...
      <Tabs
        tabs={[
          ...
          { id: 'students',   label: 'Students',   icon: 'users', count: cls.studentCount },
      ...
      {tab === 'reports' && <ClassReportsTab studentCount={cls.studentCount} quarter={cls.quarter}/>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard icon="users"       label="Total Students" value={cls.studentCount}  color="primary" sub={`${cls.section.gradeLevel} · ${cls.section.name}`}/>
        <StatCard icon="check-circle" label="Avg attendance"  value="—"               color="accent"  sub="Not yet computed"/>
        <StatCard icon="graduation-cap" label="Avg grade"     value="—"               color="blue"    sub={`${cls.quarter} cumulative`}/>
        <StatCard icon="cloud-off"   label="Pending sync"   value={0}               color="amber"   sub="Up to date"/>
      </div>

      <Card className="p-5">
        <SectionHeader title="Component weights" subtitle={`${cls.quarter} configuration`}/>
        <div className="grid grid-cols-3 gap-4 mt-3">
          {[
            { label: 'Written Works (WW)',      pct: Math.round(cls.wwWeight * 100), icon: 'pencil-line',  color: 'bg-teal-500' },
            { label: 'Performance Tasks (PT)',  pct: Math.round(cls.ptWeight * 100), icon: 'wrench',       color: 'bg-emerald-500' },
            { label: 'Quarterly Assessment (QA)', pct: Math.round(cls.qaWeight * 100), icon: 'file-check', color: 'bg-indigo-500' },
          ].map((row) => (
            <div key={row.label} className="rounded-md border border-line p-3.5">
              <div className="flex items-center gap-2">
                <Icon name={row.icon} size={14} className="text-muted"/>
                <span className="text-[12.5px] font-semibold text-navy">{row.label}</span>
              </div>
              <div className="text-[28px] font-semibold text-navy font-mono mt-2">{row.pct}%</div>
              <Progress value={row.pct} className="mt-2" barClass={row.color}/>
            </div>
          ))}
        </div>
      </Card>
...
function ClassStudentsTab({ classId }: { classId: string }) {
  ...
  const filtered = students.filter((s) =>
    !q || `${s.lastName} ${s.firstName}`.toLowerCase().includes(q.toLowerCase()) || s.lrn.includes(q),
  );
  ...
  return (
    <Card className="overflow-hidden">
      <div className="px-4 py-3 border-b border-line flex items-center gap-3 flex-wrap">
        ...
        <span className="ml-auto text-[12px] text-muted">{filtered.length} students</span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="p-8 text-center text-[13px] text-muted">
          {q ? 'No students match your search.' : 'No students enrolled in this class yet.'}
        </div>
      ) : (
...
function ClassAttendancePlaceholder() {
  return (
    <Card className="p-8 text-center">
      ...
      <p className="text-[14px] font-semibold text-navy">Attendance history coming soon</p>
      <p className="text-[13px] text-muted mt-1">Use the Attendance module to take and view attendance records.</p>
    </Card>
  );
}

function ClassReportsTab({ studentCount, quarter }: { studentCount: number; quarter: string }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {[
        { code: 'SF2', name: 'SF2 Attendance Record',  desc: 'Monthly daily attendance per learner, DepEd format.', icon: 'clipboard-list' },
        { code: 'SF9', name: 'Grade Summary',           desc: `All ${studentCount} students, all components, final ${quarter} grade.`, icon: 'graduation-cap' },
        ...
        <Card key={i} className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <h4 className="text-[14px] font-semibold text-navy">{r.name}</h4>
