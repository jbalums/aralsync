...
const IDX_DAY: Record<number, string> = {
	1: "Mon",
	2: "Tue",
	3: "Wed",
	4: "Thu",
...
function typeColor(t: ScheduleType | string) {
	return (
		{
			class: { bg: "#ECFEFF", border: "#05ab22", text: "#0F766E" },
...
export function PageSchedules() {
	...
	const [newDraft, setNewDraft] = useState<Omit<ScheduleBlock, "id"> | null>(
		null,
	);
	...
	const totalHours = useMemo(
		() =>
			schedule
				...
				.reduce((a, e) => a + e.durMin / 60, 0),
		[schedule],
	);

	// conflict check on form change
	const runConflictCheck = useCallback(
		(draft: Partial<ScheduleBlock>, excludeId?: string) => {
			clearTimeout(conflictDebounce.current);
			if (!draft.dayOfWeek || draft.startH === undefined || !draft.durMin)
				return;
				...
				try {
					...
					if (res.hasConflict) {
						const names = res.conflicts
							.map(
								(c) =>
									`${c.title} (${fmtHM(c.startH, c.startM)})`,
							)
							.join(", ");
	...
	const handleSave = async (
		form: Omit<ScheduleBlock, "id"> & { id?: string },
	) => {
		try {
			...
			if (form.id) {
				...
				toast.push({
					type: "success",
					message: `Updated · ${form.title}`,
				});
			} else {
				await createSchedule.mutateAsync(input);
				toast.push({
					type: "success",
					message: `Added to ${DAY_FULL[IDX_DAY[form.dayOfWeek]]} · ${form.title}`,
				});
			}
			setEditing(null);
			setNewDraft(null);
			...
			toast.push({
				type: "error",
				message: "Failed to save schedule block.",
			});
		}
	};
	...
	const handleImport = async () => {
		...
		for (const cl of classLoads) {
			if (!cl.schedule?.dayOfWeek?.length || !cl.schedule?.timeStart)
				continue;
			const [startH = 7, startM = 0] = cl.schedule.timeStart
				.split(":")
				.map(Number);
			const [endH = 8, endM = 0] = (cl.schedule.timeEnd ?? "")
				.split(":")
				.map(Number);
		...
		toast.push({
			...
			message: `Imported ${created} block(s) from class loads.`,
		});
	};
		...
		<div className="page-anim space-y-5">
			...
			<Card className="p-4 sm:p-5">
				<div className="flex items-start justify-between gap-3 flex-wrap">
					<div>
						...
						<p className="text-[13px] text-muted mt-1">
							{totalHours.toFixed(0)} teaching hours / week across{" "}
							{classLoads.length} class loads. Click any cell to
							add a block.
						</p>
					</div>
						...
						<div className="inline-flex rounded-md border border-line bg-white overflow-hidden">
							{(
								...
								<button
									...
									className={`px-3 h-9 text-[12.5px] font-semibold inline-flex items-center gap-1.5 tx ${view === o.id ? "bg-navy text-white" : "text-navy hover:bg-surface"}`}
								>
				...
				<div className="mt-4 flex items-center gap-3 flex-wrap text-[11.5px] text-muted">
					{(["class", "duty", "meeting", "break"] as const).map(
						(t) => (
							<span
								key={t}
							...
							>
								<span
									...
									style={{
										background: typeColor(t).bg,
										border: `1px solid ${typeColor(t).border}`,
									}}
								/>
								{
									{
			...
			{isLoading && (
				<Card className="p-6 flex items-center justify-center">
					<Icon
						name="loader-2"
						size={20}
						className="animate-spin text-muted mr-2"
					/>
						...
						>
							...
							{DAYS.map((d) => {
								...
								return (
									<div
										key={d}
										className={`bg-surface border-b border-l border-line px-3 py-2 ${isToday ? "bg-primary" : ""}`}
									>
										...
										{isToday && (
											<div className="text-[10px] text-primary-dark font-semibold mt-0.5">
												Today
											</div>
										)}
									</div>
				...
				<Card className="overflow-hidden">
					<div className="px-4 py-3 border-b border-line">
						<h3 className="text-[14px] font-semibold text-navy">
							Agenda view · {DAYS.length} days
						</h3>
					</div>
					<ul className="divide-y divide-line">
						{DAYS.filter((d) => (eventsByDay[d] ?? []).length).map(
							(d) => (
						...
						{!DAYS.some((d) => (eventsByDay[d] ?? []).length) && (
							<li className="p-8 text-center text-[13px] text-muted">
								No schedule blocks yet. Click "New block" to add
								one.
							</li>
						)}
					</ul>
			...
			{editorOpen && editorInitial && (
				<EventEditor
					...
					onDelete={
						editing
							? () => handleDelete(editing.id, editing.title)
							: undefined
					}
					onFieldChange={(draft) =>
						runConflictCheck(draft, editing?.id)
					}
				/>
			)}
			...
			>
				<ul className="divide-y divide-line">
					{classLoads.length === 0 && (
						<li className="py-4 text-[13px] text-muted text-center">
							No class loads found.
						</li>
					)}
					{classLoads.map((c) => (
						<li
...
function DayHourCell({
	...
	return (
		<div
			...
			className={`relative border-t border-l border-line h-16 group ${isToday ? "bg-blue-100/40" : ""}`}
		>
			{events.length === 0 && (
				<button
					onClick={onAdd}
			...
			{events.map((e) => {
				...
				const accent =
					e.type === "class" ? subjectHue(e.title) : col.border;
				return (
					<button
						key={e.id}
						onClick={(ev) => {
						...
						style={{
							...
							boxShadow: `0 0 1px 0 ${col.border}`,
							color: col.text,
						}}
						title={`${e.title} · ${e.section} · ${fmtHM(e.startH, e.startM)}`}
					>
						<div className="flex items-center gap-1 mb-0.5">
							<div className="text-[10.5px] font-bold tracking-tight truncate flex-1">
								{e.title}
							</div>
							{e.type === "class" && (
						...
						<div className="text-[9px] font-mono opacity-70 mt-0.5">
							{fmtHM(e.startH, e.startM)} · {e.durMin}m
						</div>
					</button>
				);
			})}
...
function EventEditor({
	...
	const selectClassLoad = useCallback(
		(cl: any) => {
			setForm((prev: any) => {
				const next = {
					...prev,
					...
					<Field label="Class" required>
						...
						{linkedClassMissing && (
							<div className="text-[11px] text-amber-700 mt-1">
								Previously linked class was removed — pick
								another.
							</div>
						)}
					</Field>
				...
				<div className="grid grid-cols-3 gap-3">
					<Field label="Day" required>
						<Select
							...
							onChange={(e) =>
								set("dayOfWeek", Number(e.target.value))
							}
						>
							{DAYS.map((d) => (
								<option key={d} value={DAY_MAP[d]}>
									{DAY_FULL[d]}
								</option>
							))}
						</Select>
					...
					<Field label="Start time" required>
						<Select
							value={timeVal}
							onChange={(e) => {
								const { startH, startM } = parseFmtHM(
									e.target.value,
								);
								setForm((prev: any) => {
					...
					<Field label="Duration">
						<Select
							value={form.durMin ?? 60}
							onChange={(e) =>
								set("durMin", Number(e.target.value))
							}
						>
				...
				{form.type === "class" && !conflictMsg && selectedClassLoad && (
					<div className="rounded-md bg-primary-light/40 border border-primary-light p-3 text-[12px] text-primary-dark flex items-center gap-2">
						...
						<span className="font-semibold">
							{selectedClassLoad.subject?.name ??
								selectedClassLoad.subjectName ??
								"-"}
						</span>
						{" · "}
						<span className="font-semibold">
							{selectedClassLoad.section?.name ??
								selectedClassLoad.sectionName ??
								"-"}
						</span>
					</div>
				)}
										...
										return (
											...
											>
												<div className="min-w-0">
													<div
														className={`text-[13px] truncate ${isSel ? "text-primary-dark font-semibold" : "text-navy font-semibold"}`}
													>
														{subName} — {secName}
													</div>
													{c.roomNumber && (
