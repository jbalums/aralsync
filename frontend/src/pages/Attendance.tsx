...
import {
	useAttendanceStore,
	canSubmitAttendance,
} from "../modules/attendance/attendanceStore";
...
export function PageAttendance() {
	...
	// ─── Queries ──────────────────────────────────────────────
	const { data: classLoads = [], isLoading: classLoadsLoading } =
		useClassLoads();
		...
		const handler = (e) => {
			...
			if (k === "arrowdown") {
				...
				if (s)
					setMark(
						s.id,
						marks[s.id] === "present" ? undefined : "present",
					);
			}
		};
					...
					<div className="flex items-center gap-3 flex-wrap">
						...
						<div className="relative">
							<Select
								...
								onChange={(e) =>
									setSelectedClassLoadId(e.target.value)
								}
								className="h-10 pr-9 font-semibold text-[13px] min-w-65"
							>
								{classLoads.map((cl) => (
									<option key={cl.id} value={cl.id}>
										{cl.subject.name} · Grade{" "}
										{cl.subject.gradeLevel} –{" "}
										{cl.section.name}
									</option>
								))}
							</Select>
					...
					<Card className="overflow-hidden">
							...
							{classLoad && (
								<>
									<span className="text-[13px] font-semibold text-navy">
										{classLoad.subject.name} · Grade{" "}
										{classLoad.subject.gradeLevel} –{" "}
										{classLoad.section.name}
									</span>
						...
						{isLoading ? (
							...
							<EmptyState
								icon="users"
								title={
									searchQuery ? "No matches" : "No students"
								}
								message={
									searchQuery
										? `No students match "${searchQuery}".`
										: "No students are enrolled in this class load yet."
								}
							/>
						) : (
				...
				<aside className="hidden lg:flex flex-col gap-4 sticky top-35 self-start">
					<Card className="p-4">
						<SectionHeader
							...
							subtitle="Press Ctrl+F or Cmd+F to search"
						/>
						<TextInput
							ref={searchInputRef}
						...
						<div className="grid grid-cols-3 gap-2">
							{[
								...
								return (
									<div
										key={r.k}
										className="rounded-md py-3 px-1"
										style={{
											background: s.bg,
											color: s.fg,
										}}
									>
										<div className="text-[11px] uppercase tracking-wider font-semibold text-center">
											{s.label}
										</div>
										<div className="text-[24px] font-semibold mt-1 leading-none text-center">
											{r.v}
										</div>
									</div>
								);
							})}
							<div className="rounded-md border border-dashed border-line p-3 text-[12px] text-muted text-center">
								<div className="text-[11px] uppercase tracking-wider font-semibold text-center">
									unmarked
								</div>
								<div className="text-[24px] font-semibold mt-1 leading-none text-center">
					...
					<Card className="p-4">
						...
						<ul className="space-y-2 text-[12.5px]">
							{[
								{
									icon: "hard-drive",
									text: "Saved instantly to your device",
								},
								{
									icon: "refresh-cw",
									text: "Queued for sync - even mid-class",
								},
								{
									icon: "cloud-upload",
									text: "Uploads when connection returns",
								},
							].map(({ icon, text }) => (
								<li
									key={icon}
			...
			>
				<div className="flex flex-col gap-4">
					<p className="text-[13.5px] text-navy leading-relaxed">
						You're outside the allowed submission window for this
						class. Saving now will record attendance with a
						timestamp that falls outside the scheduled period. This
						override is logged.
					</p>
