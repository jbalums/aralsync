...
import {
  ...
  ConnPill, Sparkbars, RingChart,
  SubjectChip, SectionHeader, Btn,
  BADGE_STYLES,
} from '../components';
...
const ACTIVITY_ICONS = { attendance: 'clipboard-check', grade: 'graduation-cap', sync: 'refresh-cw' };
...
export function PageDashboard() {
  ...
  const todayAgg = summary?.todayAggregate ?? { present: 0, late: 0, absent: 0, excused: 0, total: 0 };
  const activity = summary?.recentActivity ?? [];

  const totalStudents = classLoads.reduce((a, c) => a + c.studentCount, 0);
  const avgAttendance = classLoads.length > 0
    ? classLoads.filter((c) => c.todayAttendanceRate !== null)
        .reduce((sum, c) => sum + (c.todayAttendanceRate ?? 0), 0) /
        Math.max(classLoads.filter((c) => c.todayAttendanceRate !== null).length, 1)
    : 0;
  const avgGrade = classLoads.length > 0
    ? classLoads.filter((c) => c.avgGrade !== null)
        .reduce((sum, c) => sum + (c.avgGrade ?? 0), 0) /
        Math.max(classLoads.filter((c) => c.avgGrade !== null).length, 1)
    : 0;

  const todayPresent = todayAgg.present + todayAgg.late;
  const todayTotal = todayAgg.total;
  const pctOf = (n) => todayTotal > 0 ? Math.round((n / todayTotal) * 100) : 0;
        ...
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="text-[12px] font-semibold text-primary uppercase tracking-[0.16em]">
              {todayWeekday()} · {activeQuarter} Week {isoWeekNum()} · {todayDateLabel()}
            </div>
            ...
            <p className="text-[14px] text-muted mt-1.5 max-w-xl">
              {summaryLoading ? (
                ...
                <>
                  You&apos;re teaching{' '}
                  <span className="text-navy font-semibold">{classLoads.length} {classLoads.length === 1 ? 'class' : 'classes'}</span>{' '}
                  today across{' '}
                  <span className="text-navy font-semibold">{totalStudents} students</span>.
                  {nextSession
                    ? <> Your next session is <span className="text-navy font-semibold">{nextSession.subject}</span> at <span className="text-navy font-semibold">{nextSession.time.split('–')[0]}</span> in <span className="text-navy font-semibold">{nextSession.room || 'TBA'}</span>.</>
          ...
          <div className="hidden sm:flex flex-col items-end gap-2.5">
            ...
            <Card variant="elevated" className="px-4 py-3 flex items-center gap-3 bg-white/95 backdrop-blur">
              ...
              <div>
                ...
                <div className="text-[14px] font-semibold text-navy">{fmtLastSync(lastSyncAt)}</div>
              </div>
        ...
        {summaryLoading ? (
          ...
          <>
            <StatCard
              ...
              value={todayTotal > 0 ? `${todayPresent}/${todayTotal}` : `${totalStudents}`}
              color="primary"
            ...
            <StatCard
              ...
              value={avgAttendance > 0 ? `${Math.round(avgAttendance)}%` : '—'}
              color="accent"
            ...
            <StatCard
              ...
              value={pending}
              color="amber"
              sub={online ? 'Will sync shortly' : 'Working offline'}
            />
          </>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <Card className="lg:col-span-2 p-5">
          ...
          <div className="flex flex-col">
            {todayBlocks.length === 0 ? (
              ...
              todayBlocks.map((s, i) => {
                const c = SUBJECT_COLORS[s.subject] ?? FALLBACK_COLOR;
                const isCurrent = s.status === 'current';
                const isDone = s.status === 'done';
                return (
                  <div key={s.id} className={`flex items-stretch gap-4 py-3.5 ${i > 0 ? 'border-t border-line' : ''} ${isCurrent ? 'bg-primary-light/30 -mx-5 px-5' : ''} relative`}>
                    {isCurrent && <span className="absolute left-0 top-3 bottom-3 w-0.75 rounded-r bg-primary"/>}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[12px] text-muted flex items-center gap-1 mt-0.5"><Icon name="map-pin" size={12}/>{s.room || 'TBA'}</div>
                    </div>
          ...
          {summaryLoading ? (
            ...
            <>
              <div className="flex items-center gap-4">
                <RingChart
                  percent={todayTotal > 0 ? (todayPresent / todayTotal) * 100 : 0}
                  size={130}
                  color="#0F766E"
                  label="Present"
                />
                <div className="flex-1 space-y-2.5">
                  {[
                    { k: 'present', v: pctOf(todayAgg.present) },
                    { k: 'late',    v: pctOf(todayAgg.late) },
                    { k: 'absent',  v: pctOf(todayAgg.absent) },
                    { k: 'excused', v: pctOf(todayAgg.excused) },
                  ].map((r, i) => {
        ...
        <Card className="p-5 lg:col-span-1">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-[12px] text-muted">Last sync · {fmtLastSync(lastSyncAt)}</div>
            </div>
          </div>
          <div className="mt-4 flex items-center gap-2 text-[11px] text-muted">
            ...
            {lanPeers > 0 && (
              <span className="pill" style={{ background: '#EDE9FE', color: '#4C1D95' }}>
                <Icon name="wifi" size={10}/> {lanPeers} LAN peer{lanPeers !== 1 ? 's' : ''}
              </span>
            )}
          </div>
          <Btn
            ...
            icon="cloud-upload"
            onClick={() => toast.push({ type: 'success', title: 'Sync started', message: `Uploading ${pending} pending records…` })}
          >
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          {summaryLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[0, 1].map((i) => <Skeleton key={i} className="h-28 rounded-md"/>)}
            </div>
          ) : classLoads.length === 0 ? (
            <div className="py-6 text-center text-[13px] text-muted">No class loads found. Create a class to get started.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {classLoads.map((c) => {
                const { ww, pt, qa } = c.gradebookProgress;
                const wwPct = ww[1] > 0 ? (ww[0] / ww[1]) * 100 : 0;
                const ptPct = pt[1] > 0 ? (pt[0] / pt[1]) * 100 : 0;
                const qaPct = qa[1] > 0 ? (qa[0] / qa[1]) * 100 : 0;
                return (
                  <div key={c.id} className="border border-line rounded-md p-3.5">
                    <div className="flex items-center gap-2 mb-3">
                      <SubjectChip subject={c.subject}/>
                      <span className="text-[12px] text-muted">{c.gradeLevel} · {c.section}</span>
                    </div>
                    {[
                      { label: 'WW', n: ww, pct: wwPct, color: 'bg-teal-500' },
                      { label: 'PT', n: pt, pct: ptPct, color: 'bg-emerald-500' },
                      { label: 'QA', n: qa, pct: qaPct, color: 'bg-indigo-500' },
                    ].map((row, i) => (
        ...
        <Card className="p-5">
          ...
          {summaryLoading ? (
            ...
            <div className="py-6 text-center text-[13px] text-muted">No recent activity yet.</div>
          ) : (
                ...
                return (
                  <div key={i} className="flex items-start gap-3">
                    <span className={`w-7 h-7 shrink-0 rounded-md inline-flex items-center justify-center ${tones[a.tone] ?? tones.muted}`}>
                      <Icon name={ACTIVITY_ICONS[a.type] ?? 'activity'} size={14}/>
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] text-navy">{a.label}</div>
                      <div className="text-[11px] text-muted mt-0.5">{timeAgo(a.when)}</div>
                    </div>
                  </div>
                );
              })}
        ...
        <Card className="p-5 lg:col-span-3">
          ...
          {summaryLoading ? (
            ...
            <div className="py-6 text-center text-[13px] text-muted">No class loads to display.</div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {classLoads.map((c) => {
                const trend = c.weeklyTrend.map((v) => v ?? 0);
                const latestRate = c.todayAttendanceRate ?? c.weeklyTrend.filter(Boolean).slice(-1)[0] ?? 0;
                return (
                  <div
                    key={c.id}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[13px] font-semibold text-navy font-mono">
                        {latestRate > 0 ? `${latestRate}%` : '—'}
                      </span>
                    </div>
                    <div className="text-[12px] text-muted mt-1">{c.gradeLevel} · {c.section}</div>
                    <div className="mt-3 flex items-end justify-between">
                      <Sparkbars values={trend} height={30}/>
