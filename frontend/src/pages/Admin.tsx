...
export function PageAdmin({ setRoute }) {
  ...
  return (
    <div className="page-anim space-y-5">
      ...
      {tab === 'classes'      && <AdminClasses classes={classes} isLoading={classesLoading}/>}
...
function AdminClasses({ classes, isLoading }) {
  const [q, setQ] = useState('');
  ...
  return (
    <Card className="overflow-hidden">
      <div className="px-4 py-3 border-b border-line flex items-center gap-3 flex-wrap">
        ...
        <span className="ml-auto text-[12px] text-muted">{list.length} class{list.length !== 1 ? 'es' : ''}</span>
      </div>

      {isLoading ? (
        <div className="p-4 space-y-2">{[1,2,3,4].map(i => <Skeleton key={i} className="h-12 rounded-md"/>)}</div>
      ) : list.length === 0 ? (
        <div className="py-10"><EmptyState icon="book-marked" title="No classes found" description="No class loads exist for the current school year."/></div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-[12.5px]">
            <thead className="bg-surface text-muted text-left">
            ...
            <tbody>
              {list.map((c) => (
                <tr key={c.id} className="border-t border-line hover:bg-slate-50/40">
                  ...
                  <td className="px-3 py-2 text-navy">{c.section.gradeLevel} · {c.section.name}</td>
